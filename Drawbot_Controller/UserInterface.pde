...
class User_Interface
{
    User_Interface()
    {
        ...
        Button release = cp5.addButton("release").setPosition((int)controlRect.x, (int)controlRect.y + 290);
        release.addCallback(new CallbackListener()
        {
            public void controlEvent(CallbackEvent theEvent)
            {
                if (theEvent.getAction() == ControlP5.ACTION_RELEASED)
                {
                    botDriver.sendInstruction(new Instruction("release"));
                }
            }
        });
